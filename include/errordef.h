/****************************************************************************
  ...
  -------------------------------------------------------------------------
                ...
                $Revision: 1.71 $  $Date: 2010/08/26 09:26:22 $
...
typedef enum
{
    ...
    ,   kCopOriginalBehaviour       = 0x000A        // defines an error code for callback handlers in application which needs to keep the original behaviour in CANopen stack
...
// is there CANopen available? - otherwise only CAN driver is available
#if defined (COP_WINDOWS_DLL) || (defined (CCM_MODULE_INTEGRATION) && defined (CCM_MODULE_PDO))
...
//#if ((CCM_MODULE_INTEGRATION & CCM_MODULE_PDO) != 0)
    ...
    ,   kCopPdoErrorRxEventTimer    = 0x007C        // If an RPDO is configured with Event Timer, but PDO
                                                    ...
                                                    // with this error code - but only after next receive of the RPDO
//#endif
...
#if defined (COP_WINDOWS_DLL) || ((CCM_MODULE_INTEGRATION & (CCM_MODULE_CFGMA | CCM_MODULE_CFGMASYS)) != 0)
...
#if defined (COP_WINDOWS_DLL) || ((CCM_MODULE_INTEGRATION & CCM_MODULE_SRDO) != 0)
...
#if defined (COP_WINDOWS_DLL) || ((CCM_MODULE_INTEGRATION & CCM_MODULE_SDOM) != 0)
...
#if defined (COP_WINDOWS_DLL) || ((CCM_MODULE_INTEGRATION & CCM_MODULE_SRD) != 0) || ((CCM_MODULE_INTEGRATION & CCM_MODULE_COPMGR) != 0)
...
#endif
#endif // defined (COP_WINDOWS_DLL) || (defined (CCM_MODULE_INTEGRATION) && defined (CCM_MODULE_PDO))
