/****************************************************************************
  ...
  -------------------------------------------------------------------------
                ...
                $Revision: 1.83 $  $Date: 2011/03/07 10:24:07 $
...
// if CDRV_USE_BAUD_AUTODETECT!=FALSE and kBdiAuto ist set in cdrvinit 
// the driver will try to detect the can baudrate 
#ifndef CDRV_USE_BAUD_AUTODETECT 
    #define CDRV_USE_BAUD_AUTODETECT         FALSE 
#endif
...
#ifndef CDRVWIN_USE_CYCLIC_TX_INTERFACE
    #define CDRVWIN_USE_CYCLIC_TX_INTERFACE FALSE
#endif

#ifndef CDRV_USE_SETBAUDRATE_FUNC
    #define CDRV_USE_SETBAUDRATE_FUNC       TRUE
#endif
...
// -------------------------------------------------------------------------
typedef tFastByte           tCdrvIsrType;
#define kCdrvRxIsr          0
#define kCdrvTxIsr          1
#define kCdrvErrorIsr       2
#define kCdrvRxFIFO0Isr     3
#define kCdrvRxFIFO1Isr     4


// -------------------------------------------------------------------------
// constants for baud rate (CiA DS-305)

typedef tFastByte           tCdrvBaudIndex;
#define kBdi1MBaud          0       // 1 MBit/sec
#define kBdi800kBaud        1       // 800 kBit/sec
#define kBdi500kBaud        2       // 500 kBit/sec
#define kBdi250kBaud        3       // 250 kBit/sec
#define kBdi125kBaud        4       // 125 kBit/sec
#define kBdi100kBaud        5       // 100 kBit/sec
#define kBdi50kBaud         6       // 50 kBit/sec
#define kBdi20kBaud         7       // 20 kBit/sec
#define kBdi10kBaud         8       // 10 kBit/sec
#define kBdiAuto           -1       // automatic baud rate detection (not implemented in each CDRV module)
#define kBdiListenOnlyMode  0x80    // r.d.: do not use in this version!!!


// -------------------------------------------------------------------------
// Bits for comparision of message types (refer to tMsgTyp).
typedef tFastByte           tMsgTypBit;
...
#define kMtbCyclicMsg       0x08    // only for using USB-CANmodul with Cdrvwin.c and Cdrvwrap.dll
#define kMtbReserved        0x10    // cannot be used (is reserved for kCobTypRtrInvalid)
...
//#if ((TARGET_SYSTEM == _LINUX_) && ((TARGET_HARDWARE & TGT_ADD_EMASK_) == TGT_ADD_PEAK))
    ...
    #if((TARGET_HARDWARE & TGT_ADD_EMASK_) == TGT_ADD_PEAK)
//#ifdef LINUX
...
// hardware parameter for Windows and Peak CAN hardware
        ...
        typedef struct
        {
        #if ((TARGET_HARDWARE & TGT_ADD_EMASK_) == TGT_ADD_ESD_NTCAN)
            BYTE            m_bThrdPriority;
            int             m_nNetNr;
            HANDLE          m_hCanMsgReceive;
        #elif ( ((TARGET_HARDWARE & TGT_ADD_MASK_) == TGT_ADD_TDRV011) || \
                ((TARGET_HARDWARE & TGT_ADD_MASK_) == TGT_ADD_UCAN)         )
            // TGTHW_LIS_IPC_TDRV011
            BYTE            m_bThrdPriority;
            BYTE            m_bDeviceNr;
            BYTE            m_bChannel;
            HANDLE          m_hCanMsgReceive;
            ...
            BYTE            m_bIpProtocol;
            BYTE            m_bChannel;         // CAN-Channel for USBCANmodul and CAN-Ethernet-Gateway with two CAN channels
            DWORD           m_dwUserBaudrate;   // only for special CAN drivers (as such as Cdrv_vcan_sio.c)
        #endif
        }
        tWindowsParam;
    #endif
...
typedef struct
{
    BYTE HWACC*     m_pbBaseAddr;   // base address of CAN controller
        ...
        PxTask_t*   m_pbTaskId;     // pointer task-ID of application task
    #endif
...
#define kCdrvOverrun            0x0020          // hardware overrun (e.g. in CAN controller or UART)
...
#define kCdrvTunnelTxOverrun    0x2000  // only virtual CAN
...
// This type is used by the application, COB module and canlindev.c.
// It was moved from Cop.h to this place and renamed from tCobCdrvFct to tCdrvFct.
typedef struct
...
//---------------------------------------------------------------------------
//
// Function:    CdrvGetFileDescriptor()
//
// Description: Returns the POSIX file descriptor of the underlying character
//              device. It may be used by the application for select()/poll()
//              calls.
//
// Note:        This function is only implemented by canlinstub.c!
//
// Parameters:  MCO_DECL_INSTANCE_PTR
//
...
CDRVDLLEXPORT int PUBLIC CdrvGetFileDescriptor (MCO_DECL_INSTANCE_PTR);
